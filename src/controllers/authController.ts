...
export async function cadastrar(req: Request, res: Response) {
  try {
    ...
    res.status(201).json({
      token,
      usuario: { id_usuario: novoUsuario.id_usuario, nm_usuario: novoUsuario.nm_usuario, email_usuario: novoUsuario.email_usuario },
    });
  } catch (err) {
...
export async function login(req: Request, res: Response) {
  try {
    ...
    res.status(200).json({
      token,
      usuario: { id_usuario: usuario.id_usuario, nm_usuario: usuario.nm_usuario, email_usuario: usuario.email_usuario },
    });
  } catch (err) {
...
export async function buscarPerfil(req: Request, res: Response) {
  try {
    ...
    // nunca devolve a senha_hash, mesmo sendo o hash — não é dado que o app precisa ver
    res.status(200).json({
    ...
    console.error('[authController.buscarPerfil]', err);
